<template >
  <v-stepper v-model="e6" vertical>
    ...
    <v-stepper-step id="sekcijaSedista" :complete="e6 > 2" step="2"
      >Izabir sedista</v-stepper-step
    >

    <v-stepper-content step="2">
      <div
        v-for="(sediste, index) in sedista"
        :key="sediste.redniBroj"
        style="display: inline; width: fit-content; margin: 0"
      >
        <br v-if="index % 10 == 0" />
        {{ sediste.redniBroj }}
        <b-form-checkbox
          :label="sediste.redniBroj"
          :value="sediste.redniBroj"
          :disabled="sediste.disabled"
          @change="testttt(sediste.redniBroj)"
          style="display: inline; width: fit-content; margin: 0"
        ></b-form-checkbox>
      </div>
    ...
    <v-stepper-content step="3">
      ...
      Odabrana sedista:
      <div v-for="a in this.selektovanaSedista" :key="a">
        <p>{{ a }}</p>
      </div>
      <br />
      Ukupna cena:
      ...
      <v-card color="grey lighten-1" class="mb-12" height="200px"></v-card>
      <v-btn class="bluebtn" color="success" @click="dodavanjeKarte"
        >Plati</v-btn
      >
      <v-btn text>Cancel</v-btn>
    </v-stepper-content>
...
export default {
  ...
  mounted() {
    ...
    axios
      ...
      .then((res) => {
        ...
        for (var i = 0; i < res.data.sedista.length; i++) {
          this.sedista.push({
            id: res.data.sedista[i].redniBroj,
            disabled: false,
            redniBroj: res.data.sedista[i].redniBroj,
          });
        }
      })
    ...
    axios
      .get(
        `http://localhost:8081/SF-15-2018-OWP/ZauzetaSedista?id_projekcije=${this.id}`
      )
      .then((res) => {
        console.log("hi");
